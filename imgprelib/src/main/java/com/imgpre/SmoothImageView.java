...
public class SmoothImageView extends PhotoView {
                    ...
                    if (event.getPointerCount() == 1) {
                        ...
                        if (!isMoved && (Math.abs(offsetX) > Math.abs(offsetY) || Math.abs(offsetY) < MIN_TRANS_DEST)) {
                            ...
                            if (onTransformingListener!=null){
                                onTransformingListener.onTransforming(offsetX,offsetY);
                            }
                ...
                case MotionEvent.ACTION_CANCEL:
                    if (isMoved) {
                        if (moveScale() <= MAX_TRANS_SCALE) {
                            moveToOldPosition();
                            if (onTransformingListener!=null){
                                onTransformingListener.onTransFormFinish(0f,0f);
                            }
                        } else {
                            changeTransform();
    ...
    private OnTransformingListener onTransformingListener;

    public void setOnTransformingListener(OnTransformingListener onTransformingListener) {
        this.onTransformingListener = onTransformingListener;
    }

    public interface OnTransformingListener{
        void onTransforming(float offsetX,float offsetY);
        void onTransFormFinish(float offsetX,float offsetY);
    }
